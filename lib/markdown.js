...
export const renderers = {
  ...
  blockquote: props => <blockquote className='bg-gray-100 px-4 py-6 mb-8 border-l-2'>{ props.children }</blockquote>,
};

const renderHeading = (props) => {
  switch (props.level) {
    case 1:
      return <h1 className='font-bold mb-12 text-2xl p-2 border-b-4 border-t-4 border-gray-700'>{ props.children }</h1>
    case 2:
      return <h2 className='font-bold mt-20 mb-4 text-xl border-l-8 border-gray-700 px-3 py-2' style={{borderBottom: '1px solid gray' }}>{ props.children }</h2>
    case 3:
...
const renderList = (props) => {
  if(props.ordered) {
    return <ol className='text-left list-decimal ml-8 mb-8'>{props.children}</ol>;
  }else {
    return <ul className='text-left list-disc ml-8 mb-8'>{props.children}</ul>;
  }
}
